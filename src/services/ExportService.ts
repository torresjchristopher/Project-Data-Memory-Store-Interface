 ...
 * Fixes: Ultra-robust fetch and strict folder-per-person sorting.
 */
  ...
  async exportAsZip(
    ...
    const downloadPromises = (tree.memories || []).map(async (memory) => {
      ...
      try {
        ...
        // CORS Safety: Force mode 'cors' and add cache-buster
        const archivalUrl = `${memory.photoUrl}${memory.photoUrl.includes('?') ? '&' : '?'}_archival_id=${Date.now()}`;
        ...
        if (blob.size < 100) throw new Error("Artifact blob too small, likely corrupted or blocked");
    ...
    console.log(`📦 [EXPORT] Composition Complete. Total: ${successCount}`);
