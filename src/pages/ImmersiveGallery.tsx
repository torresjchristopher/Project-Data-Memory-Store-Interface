...
export default function ImmersiveGallery({ tree, onExport }: ImmersiveGalleryProps) {
  ...
  const saveEdit = async () => {
    ...
    // 2. Sync to Firestore (with error check)
    try {
      await PersistenceService.getInstance().saveMemorySync(updatedMemory, tree.protocolKey || 'MURRAY_LEGACY_2026');
    } catch (err) {
      console.error("Database Sync Error:", err);
      alert("Permission Error: Please update your Firestore Security Rules to allow writes.");
    }
  };
      ...
      <AnimatePresence>{showCli && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] bg-black/98 backdrop-blur-2xl overflow-y-auto">
          <div className="p-8 md:p-12">
            <button onClick={() => setShowCli(false)} className="fixed top-8 right-8 p-4 bg-white/5 rounded-full shadow-2xl hover:bg-white/10 transition-all"><X className="w-6 h-6 text-white" /></button>
      ...
      <div className="relative z-10 w-full h-screen flex flex-col">
        ...
        {filteredMemories.length === 0 ? (
          <div className="flex-1 flex flex-col items-center justify-center p-10 text-center">
            <p className="text-white/20 font-serif italic mb-8 text-xl">No fragments match the search protocol.</p>
            <button onClick={() => { setSearchQuery(''); setFilterPerson(''); }} className="px-10 py-4 border border-white/10 text-white text-[10px] font-black uppercase hover:bg-white hover:text-black transition-all">Clear Search</button>
          </div>
        ) : (
          <div className="flex-1 relative flex items-center justify-center overflow-hidden">
            {viewMode === 'theatre' && currentMemory && (
              <>
                ...
                >
                  <div className="relative w-96 min-h-[130px] preserve-3d shadow-[0_30px_60px_rgba(0,0,0,0.8)] bg-black/90 backdrop-blur-3xl border border-white/10 px-10 py-8 rounded-sm flex flex-col items-center justify-center text-center">
                    <div 
